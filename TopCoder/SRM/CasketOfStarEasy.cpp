...
int hoge( VI weight, int t, int depth ){
    ...
    rep(i,weight.size()-2){
        ...
        int a,b;
        for( int j = pos+1; j < weight.size(); ++j ) if( dis[j]==0 ){ a=weight[j];break;}
        for( int j = pos-1; j > -1; --j ) if( dis[j]==0 ){ b=weight[j];break;}

        dis[pos] = 1;
        maxr = max(hoge( weight, t + a*b, depth+1 ),maxr);
        dis[pos] = 0;
    }
}
